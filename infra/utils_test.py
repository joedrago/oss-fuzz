...
class BuildFuzzersTest(unittest.TestCase):
  ...
  def test_build_fuzzers_from_commit(self):
    ...
    should not.
    """
    test_data = os.path.join(TEST_DIR_PATH, 'yara_test_data')
...
class DetectMainRepoTest(unittest.TestCase):
  """Testing the ability to detect an OSS-Fuzz projects main repo."""

  def test_detect_main_repo_none(self):
    """Tests that when a incorrect arguments are given, None is returned."""
    repo_origin, repo_name = utils.detect_main_repo('curl')
    self.assertIsNone(repo_origin)
    self.assertIsNone(repo_name)

  def test_detect_main_repo_from_commit(self):
    """Test the detect main repo functionality of the utils module."""
  ...
  def test_detect_main_repo_from_name(self):
    ...
    # Testing a repo name that does not exist.
    repo_origin, repo_name = utils.detect_main_repo(
